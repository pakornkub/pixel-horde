...
export const st = (s: SimState, id: SkillId, lv: number): SkillStats => skillStats(id, lv, !!s.P.evo[id]);
...
export function useUlt(s: SimState): void {
  if (s.phase !== 'play' || s.ult < ULT_MAX) return;
  s.ult = 0;
  const targets = visibleEnemies(s);
  const dmg = (160 + 45 * s.P.lv) * ipow(1.45, s.stage - 1);
...
export function updSkills(s: SimState, dt: number): void {
  const P = s.P, sk = P.skills, R = s.rng.skills;
  for (const id of Object.keys(sk) as SkillId[]) {
    ...
    if (id === 'bolt') {
      ...
      for (let i = 0; i < t.n; i++) {
        ...
        s.bolts.push({ kind: 'bolt', x: P.x, y: P.y - 3, vx: cos(a) * 200, vy: sin(a) * 200, life: 1.1, dmg: t.dmg, pierce: t.pierce, hit: new Set(), col: '#ff5cf4', rad: 3, kb: 35 });
      }
      cloneCast(s, id, t);
      P.cds[id] = t.cd * P.cdMul;
    } else if (id === 'chain') {
      const first = nearest(s, P.x, P.y, 150);
      ...
      for (const e of set) hit(s, e, t.dmg, '#fff35c', 25);
      ...
      s.effects.push({ type: 'nova', x: P.x, y: P.y, R: t.r, t: 0, dur: 0.38, hit: new Set(), dmg: t.dmg });
      shake(s, 2.5);
      sfx(s, 'nova');
      cloneCast(s, id, t);
      P.cds[id] = t.cd * P.cdMul;
      ...
      for (let i = 0; i < t.n; i++) {
        ...
        s.effects.push({ type: 'meteor', x: e.x + R.range(-6, 6), y: e.y + R.range(-6, 6), t: 0, dur: 0, delay: 0.55 + i * 0.11, r: t.r, dmg: t.dmg, boomed: false, bt: 0 });
      }
      cloneCast(s, id, t);
      P.cds[id] = t.cd * P.cdMul;
    } else if (id === 'lance') {
      if (!nearest(s, P.x, P.y, 220)) { P.cds[id] = 0.1; continue; }
      const a0 = atan2(P.dy, P.dx);
      for (let i = 0; i < t.n; i++) {
        const a = a0 + (i - (t.n - 1) / 2) * 0.22;
        s.bolts.push({ kind: 'lance', x: P.x, y: P.y - 3, vx: cos(a) * 280, vy: sin(a) * 280, a, life: 0.9, dmg: t.dmg, pierce: Infinity, hit: new Set(), col: '#ffe9a8', rad: 4, kb: 20 });
      }
      sfx(s, 'lance');
      cloneCast(s, id, t);
      P.cds[id] = t.cd * P.cdMul;
    } else if (id === 'boomer') {
      const list = nearestN(s, t.n, 180);
      if (!list.length) { P.cds[id] = 0.15; continue; }
      for (let i = 0; i < t.n; i++) {
        const e = list[i % list.length];
        const a = atan2(e.y - P.y, e.x - P.x) + (i >= list.length ? R.range(-0.5, 0.5) : 0);
        s.bolts.push({ kind: 'boom', x: P.x, y: P.y - 3, vx: cos(a) * 170, vy: sin(a) * 170, spd: 170, d: 0, range: t.range, ret: false, life: 3, dmg: t.dmg, pierce: Infinity, hit: new Set(), col: '#7dffb0', rad: 5, kb: 30, spin: 0 });
      }
      cloneCast(s, id, t);
      P.cds[id] = t.cd * P.cdMul;
    } else if (id === 'cyclone') {
      ...
      for (let i = 0; i < t.n; i++) {
        const a = R.next() * TAU;
        s.effects.push({ type: 'cyclone', x: P.x, y: P.y, vx: cos(a) * 48, vy: sin(a) * 48, t: 0, dur: t.dur, r: t.r, dmg: t.dmg, tick: 0 });
      }
      P.cds[id] = t.cd * P.cdMul;
    } else if (id === 'toxic') {
      const vis = visibleEnemies(s);
      ...
      const vis = visibleEnemies(s);
      if (vis.length < 3) { P.cds[id] = 0.3; continue; }
      let best = vis[0], bc = -1;
      for (let k = 0; k < 8; k++) {
      ...
      s.effects.push({ type: 'hole', x: best.x, y: best.y, t: 0, dur: 1.9, r: t.r, dmg: t.dmg, boom: t.boom, tick: 0, boomed: false, bt: 0 });
      P.cds[id] = t.cd * P.cdMul;
    }
  }
  if (sk.orbit) {
  ...
  if (sk.frost) {
    ...
    for (const e of s.enemies) {
      ...
      if (dx * dx + dy * dy < t.r * t.r) { e.slowT = 0.4; if (t.freeze) e.frz = 0.4; }
    }
    if (s.frostT <= 0) {
      s.frostT = 0.4;
      for (const e of s.enemies) {
        if (e.dead) continue;
        const dx = e.x - P.x, dy = e.y - P.y;
        if (dx * dx + dy * dy < t.r * t.r) hit(s, e, t.dmg, '#9fd8ff', 6);
      }
    }
  }
}
  ...
  for (const f of s.effects) {
    ...
    if (f.type === 'nova') {
      ...
      for (const e of s.enemies) {
        ...
        if (d < r + e.r) { f.hit!.add(e); hit(s, e, f.dmg, '#ff8a3d', 90); }
      }
      ...
      if (!f.boomed && f.t >= f.delay!) {
        ...
        for (const e of s.enemies) {
          if (e.dead) continue;
          if (hypot(e.x - f.x, e.y - f.y) < f.r! + e.r) hit(s, e, f.dmg, '#ff4b3a', 70);
        }
      ...
      for (const [ba, hs] of beams) {
        for (const e of s.enemies) {
          ...
          if (Math.abs(da) < 0.12 + (e.r + 3) / d) { hs.add(e); hit(s, e, f.dmg, '#5cf4ff', 50); burst(s, e.x, e.y, '#bff9ff', 3, 50, 0.3); }
        }
      }
      shake(s, 1.5);
    } else if (f.type === 'hole') {
      if (!f.boomed) {
        ...
        if (f.t >= f.dur - 0.3) {
          ...
          for (const e of s.enemies) {
            if (e.dead) continue;
            if (hypot(e.x - f.x, e.y - f.y) < f.r! * 1.1 + e.r) hit(s, e, f.boom!, '#d9b8ff', 140);
          }
      ...
      if (!f.fired && f.t >= 0.3) {
        ...
        for (const o of f.targets!) {
          if (!o.e.dead) { o.x = o.e.x; o.y = o.e.y; hit(s, o.e, f.dmg, '#fff35c', 120); }
