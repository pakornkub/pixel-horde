...
describe('balance pass 2026-09', () => {
  ...
  it('2026-09b (= v5: the fixes plus the Director change) is valid on top of 2026-09 (config v4) and only sets fields v4 left at version 0', () => {
    ...
    expect(changed.sort()).toEqual(['shared.awaken.keep', 'shared.awaken.slots', 'shared.director.max', 'shared.director.rise', 'shared.heroes.ranger.hp', 'shared.scaling.lvCapBase', 'shared.scaling.lvCapPerCh', 'shared.skills.hawk.guardN'].sort());
    for (const p of changed) expect(get(v4, p)).toBe(get(DEFAULT_CONFIG, p));
    expect(BALANCE_PASSES.map((p) => p.id)).toEqual(['2026-09b', '2026-09']); // newest first
  });
});
