...
create table public.runs (
  ...
  gold_earned int,
  score bigint,
  -- Endless (after beating Umbra in the same Run) has its own score and board
...
create or replace function public.start_run(p_hero text, p_mode text default 'solo', p_world text default 'lumora', p_weapon text default 'judgement')
returns jsonb language plpgsql security definer set search_path = '' as $$
declare
...
begin
  ...
  -- Weapons: Judgement is free; others must be owned in this World ("lumora:thornwhip")
  if coalesce(p_weapon, 'judgement') <> 'judgement' and not ((p_world || ':' || p_weapon) = any (m.weapons)) then
...
/**
 * Weapons found in a Run join the World's collection. At most 2 per Run and only well-formed
 * ids ("<world>:<name>"); duplicates are ignored.
 */
create or replace function public.add_found_weapons(uid uuid, p_world text, p_found jsonb)
returns public.meta_progress language plpgsql security definer set search_path = '' as $$
declare
  m public.meta_progress;
  add text[];
begin
  select coalesce(array_agg(distinct p_world || ':' || w), '{}') into add
  from (select w from jsonb_array_elements_text(coalesce(p_found, '[]'::jsonb)) as w
        where w ~ '^[a-zA-Z]{2,24}$' and w <> 'judgement' limit 2) f;
  update public.meta_progress
    set weapons = (select coalesce(array_agg(distinct x order by x), '{}') from unnest(weapons || add) x), updated_at = now()
  where user_id = uid returning * into m;
  return m;
end $$;
revoke all on function public.add_found_weapons(uuid, text, jsonb) from public, anon, authenticated;
...
declare f text;
begin
  foreach f in array array['get_meta()', 'start_run(text, text, text, text)', 'submit_run(jsonb)', 'submit_offline_run(jsonb)',
