...
import { BALANCE_PASSES, FIELD_TH, GROUP_TH, listFields, parseBalanceConfig, withOverrides, type BalanceConfig, type BalancePass, type FieldInfo } from '@pixel-horde/config';
...
export function Balance({ api, focus }: { api: AdminApi; focus?: string }) {
  ...
  /** A playtest pass (packages/config/src/balance-pass.ts) on top of the current draft, to review and publish. */
  const loadPass = (pass: BalancePass): void => {
    const rest = { ...(d as Record<string, unknown>) };
    delete rest.version;
    const next = withOverrides(parseBalanceConfig(rest), pass.patch) as BalanceConfig & Record<string, unknown>;
    setDraft({ ...next, version: base.version });
    if (!note.trim()) setNote(pass.note);
    toast(`ใส่ค่าจากรอบจูน ${pass.id} ในฉบับร่างแล้ว ตรวจก่อน publish`);
  };
  ...
  return (
    <div class="lab">
      ...
      <div class="stage">
        ...
        <div class="small mut">ค่าที่แนะนำจากการทดสอบด้วยบอท ใส่ทีละรอบตามลำดับ (2026-09 = v4 ที่ใช้อยู่แล้ว; 2026-09b = Awakening, HP มอนตามเลเวล, Kit)</div>
