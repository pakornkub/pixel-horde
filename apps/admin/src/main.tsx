...
const PAGES: [string, string][] = [
  ...
  ['leaderboard', 'Leaderboard'], ['players', 'ผู้เล่น'], ['feedback', 'ความเห็นผู้เล่น'], ['work', 'งานแก้ไข'], ['announce', 'ประกาศ'], ['audit', 'บันทึกการแก้ไข'],
];
...
function App() {
  ...
  const body = r.page === 'balance' ? <Balance api={api} focus={r.arg} /> : r.page === 'ai' ? <Ai api={api} go={go} /> : r.page === 'flags' ? <Flags api={api} /> : r.page === 'stats' ? <Stats api={api} />
    : r.page === 'leaderboard' ? <Leaderboard api={api} initial={r.arg} /> : r.page === 'players' ? <Players api={api} /> : r.page === 'feedback' ? <Feedback api={api} go={go} /> : r.page === 'announce' ? <Announce api={api} />
